...
class BooheeSingleSpider(scrapy.Spider):
    name = 'boohee_single'
    allowed_domains = ['www.boohee.com']
    start_urls = ['https://www.boohee.com/shiwu/chaodoufusong2']
    ...
    def parse(self, response):
        ...
        for related_food in related_food_list:
            ...
            # yield scrapy.Request(url=food_url, callback=self.parse)
            ...
            for node in widget_more_list:
                ...
                if "h3" in node_str:
                    ...
                    if node.xpath('ul'):
                        ...
                        item['instruction'] = node.xpath('p').extract_first()
